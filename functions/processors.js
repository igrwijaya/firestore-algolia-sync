...
const DocumentReference = fbAdmin.firestore.DocumentReference;
const GeoPoint = fbAdmin.firestore.GeoPoint;
const Timestamp = fbAdmin.firestore.Timestamp;
...
const processArray = (arrayVal) => {
  return arrayVal.map((val, index) => {
    if (val instanceof Object) {
      const [value] = processValue(index, val);
      return value;
    }
    return val;
  });
